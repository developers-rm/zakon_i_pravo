    ...
    public static function MakeContent($vParam, $vShab, $view) {
        if ($view)
            switch ($view) {
                ...
                break;
                case 'NO_TEMPLATE':
                    return view::template(ROOT_PATH . '' . $vShab['content'], (array) $vParam + array('shablon_name' => $vShab));
                break;
                default :
                    return view::template(ROOT_PATH . $vShab['index'], (array) $vParam + array('shablon_name' => $vShab));
            } else {
            return @view::template(ROOT_PATH . $vShab['index'], (array) $vParam + array('shablon_name' => $vShab));
        }
    }
